...
export default function DashBoard({ initialTab }) {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    const validTabs = ['overview', 'quizzes', 'mocks', 'analytics', 'blogs'];
    ...
    <div className={`flex min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-gray-50' : 'bg-gray-50 text-gray-900'}`}>
        ...
        <nav>
          <ul className="space-y-1">
            <li>
              ...
              >
                {sidebarCollapsed ? <span>📊</span> : <><span className="mr-2">📊</span><span className="text-sm">Overview</span></>}
              </button>
            </li>
            <li>
              <button
              ...
              >
                {sidebarCollapsed ? <span>🏆</span> : <><span className="mr-2">🏆</span><span>Mocks</span></>}
              </button>
            </li>
            <li>
              <button
              ...
              >
                {sidebarCollapsed ? <span>📝</span> : <><span className="mr-2">📝</span><span>Quizzes</span></>}
              </button>
            </li>
            <li>
              <button
              ...
              >
                {sidebarCollapsed ? <span>📑</span> : <><span className="mr-2">📑</span><span>Blogs</span></>}
              </button>
            </li>
            <li>
              <button
              ...
              >
                {sidebarCollapsed ? <span>📈</span> : <><span className="mr-2">📈</span><span>Analytics</span></>}
              </button>
            </li>
          </ul>
        </nav>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-10">
        <div className={`flex justify-around ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-1 shadow`}>
          <button onClick={() => setActiveTab('overview')} className={`p-2 text-xs ${activeTab === 'overview' ? (theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600') : ''}`}>
            <div className="flex flex-col items-center"><span>📊</span><span>Home</span></div>
          </button>
          <button onClick={() => setActiveTab('quizzes')} className={`p-2 text-xs ${activeTab === 'quizzes' ? (theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600') : ''}`}>
            <div className="flex flex-col items-center"><span>📝</span><span>Quizzes</span></div>
          </button>
          <button onClick={() => setActiveTab('mocks')} className={`p-2 text-xs ${activeTab === 'mocks' ? (theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600') : ''}`}>
            <div className="flex flex-col items-center"><span>🏆</span><span>Mock</span></div>
          </button>
          <button onClick={() => setActiveTab('blogs')} className={`p-2 text-xs ${activeTab === 'blogs' ? (theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600') : ''}`}>
            <div className="flex flex-col items-center"><span>📑</span><span>Blogs</span></div>
          </button>
           <button onClick={() => setActiveTab('analytics')} className={`p-2 text-xs ${activeTab === 'analytics' ? (theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600') : ''}`}>
            <div className="flex flex-col items-center"><span>📈</span><span>Analytics</span></div>
          </button>
        </div>
      </div>

      <div className={`flex-1 p-3 transition-all duration-200`}>
        {activeTab === 'overview' && (
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={participationData}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#1e293b' : '#fff', borderColor: theme === 'dark' ? '#334155' : '#cbd5e1', fontSize: '12px' }} />
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className={`text-left ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                        <th className="pb-1">User</th><th className="pb-1">Quiz</th><th className="pb-1">Score</th><th className="pb-1">Time</th>
                      </tr>
                    </thead>
                    <tbody>
                      {recentData.map((item, index) => (
                        <tr key={index} className={theme === 'dark' ? 'border-b border-gray-700' : 'border-b border-gray-200'}>
                          ...
                          <td className={`py-2 font-medium ${item.score >= 70 ? 'text-green-500' : item.score >= 50 ? 'text-yellow-500' : 'text-red-500'}`}>{Math.round(item.score)}%</td>
        ...
        {activeTab === 'quizzes' && <Quizes quizes={quizes} setQuizes={setQuizes} getQuizes={getQuizes} />}
        {activeTab === 'mocks' && (
        ...
        {activeTab === 'blogs' && <div className="overflow-auto w-full"><BlogManager /></div>}
        {activeTab === 'analytics' && (
          <div>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={quizStats.performanceData}>
                      <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#64748b' : '#cbd5e1'} />
                      <XAxis dataKey="name" stroke={theme === 'dark' ? '#fff' : '#000'} fontSize={10} /><YAxis stroke={theme === 'dark' ? '#fff' : '#000'} fontSize={10} />
                      <Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#1e293b' : '#fff', borderColor: theme === 'dark' ? '#334155' : '#cbd5e1', fontSize: '12px' }} />
