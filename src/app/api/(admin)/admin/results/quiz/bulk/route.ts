...
export async function POST(request: NextRequest) {
  try {
    ...
    const deletion = await Attempted.deleteMany({ _id: { $in: ids } });

    return NextResponse.json({
      success: true,
      deletedCount: deletion.deletedCount || 0,
      message: `Deleted ${deletion.deletedCount || 0} quiz result(s) successfully`,
    });
  } catch (error) {
    console.error('Failed to bulk delete quiz results:', error);
    return NextResponse.json({ success: false, message: 'Failed to bulk delete quiz results' }, { status: 500 });
  }
}
